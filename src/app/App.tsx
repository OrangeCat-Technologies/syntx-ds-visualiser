...
export const toolName = "DS Visualiser";
...
const TableMenu: React.FC = () => {
  ...
  const [openDialog, setOpenDialog] = useState<
    "database" | "extract" | "paste" | "upload" | null
  >(null);
  const fileInputRef = React.useRef<HTMLInputElement>(null);
  const open = Boolean(anchorEl);

  const handleOpenDialog = (
    dialog: "database" | "extract" | "paste" | "upload"
  ) => {
    setAnchorEl(null);
    if (dialog === "upload") {
  ...
  return (
    <>
      ...
      <TableCopyDialogV2
        ...
        onClose={() => setOpenDialog(null)}
      />
      <TableUploadDialog fileInputRef={fileInputRef} />
    </>
  );
};
...
export const AppFC: FC<AppFCProps> = function AppFC(appProps) {
  ...
  let router = createBrowserRouter([
    ...
    {
      path: "/",
      element: serverConfig.PROJECT_FRONT_PAGE ? (
        <About />
      ) : (
        <DataFormulatorFC />
      ),
