...
let deleteChartsRoutine = (state: DataFormulatorState, chartIds: string[]) => {
  ...
  let tableIdsToDelete = state.tables
...
export const fetchCodeExpl = createAsyncThunk(
  ...
  async (derivedTable: DictTable, { getState }) => {
    ...
    let response = await fetch(getUrls().CODE_EXPL_URL, {
      ...message,
      signal: controller.signal,
    });
...
export const fetchAvailableModels = createAsyncThunk(
  ...
  async () => {
    ...
    let response = await fetch(getUrls().CHECK_AVAILABLE_MODELS, {
      ...message,
      signal: controller.signal,
    });
...
export const dataFormulatorSlice = createSlice({
  ...
  reducers: {
    ...
    loadState: (state, action: PayloadAction<any>) => {
      ...
      state.focusedDataCleanBlockId =
        savedState.focusedDataCleanBlockId || undefined;
    ...
    deleteTable: (state, action: PayloadAction<string>) => {
      ...
      let chartIdsToDelete = state.charts
        .filter((c) => c.tableRef == tableId)
        .map((c) => c.id);
    ...
    extendTableWithNewFields: (
      ...
      if (previousName && table.names.indexOf(previousName) != -1) {
        ...
        let insertPosition = lastParentName
          ? table.names.indexOf(lastParentName)
          : table.names.length - 1;
        newNames = table.names
          .slice(0, insertPosition + 1)
          .concat(columnName)
          .concat(table.names.slice(insertPosition + 1));
      }

      let newMetadata = structuredClone(table.metadata);
      for (let name of newNames) {
        newMetadata[name] = {
          type: inferTypeFromValueArray(newValues),
          semanticType: "",
          levels: [],
        };
      }
    ...
    removeDerivedField: (
      ...
      let table = state.tables.find((t) => t.id == tableId) as DictTable;
      let fieldName = state.conceptShelfItems.find((f) => f.id == fieldId)
        ?.name as string;
      ...
      if (fieldIndex != -1) {
        table.names = table.names
          .slice(0, fieldIndex)
          .concat(table.names.slice(fieldIndex + 1));
      ...
      if (chart) {
        ...
        if (prop == "fieldID") {
          ...
          if (
            table &&
            field &&
            table.metadata[field.name] &&
            table.metadata[field.name].levels &&
            table.metadata[field.name].levels.length > 0
          ) {
    ...
    swapChartEncoding: (
      ...
      if (chart) {
        ...
        chart.encodingMap[channel1] = {
          fieldID: enc2.fieldID,
          aggregate: enc2.aggregate,
          sortBy: enc2.sortBy,
          sortOrder: enc2.sortOrder,
        };
        chart.encodingMap[channel2] = {
          fieldID: enc1.fieldID,
          aggregate: enc1.aggregate,
          sortBy: enc1.sortBy,
          sortOrder: enc1.sortOrder,
        };
      }
    },
    ...
    clearUnReferencedCustomConcepts: (state) => {
      ...
      let fieldIdsReferredByCharts = dfSelectors
        .getAllCharts(state)
        .map(
          (c) =>
      ...
      .addCase(fetchFieldSemanticType.fulfilled, (state, action) => {
        ...
        if (data["status"] == "ok" && data["result"].length > 0) {
          ...
          if (data["result"][0]["suggested_table_name"]) {
            ...
            let existingDisplayIds = state.tables
              .filter((t) => t.id != tableId)
              .map((t) => t.displayId);
            let suffix = "";
            let displayId = `${
              data["result"][0]["suggested_table_name"] as string
            }${suffix}`;
            let suffixId = 1;
...
export const dfSelectors = {
  getActiveModel: (state: DataFormulatorState): ModelConfig => {
    return (
      state.models.find((m) => m.id == state.selectedModelId) || state.models[0]
    );
  },
  ...
  replaceChart: (state: DataFormulatorState, chart: Chart) => {
    if (state.charts.find((c) => c.id == chart.id)) {
      ...
      if (table.derive?.trigger) {
        table.derive = {
          ...table.derive,
          trigger: { ...table.derive?.trigger, chart: chart },
        };
      }
    }
  },
...
export const getDataFieldItems = (baseTable: DictTable): FieldItem[] => {
  let dataFieldItems =
    baseTable.names.map((name, index) => {
