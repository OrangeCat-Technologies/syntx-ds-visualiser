...
export const LittleConceptCard: FC<LittleConceptCardProps> =
  function LittleConceptCard({
    channel,
    field,
    encoding,
    handleUnbind,
    ...
    return (
      <Chip
        ...
        sx={{
          ...
          cursor: cursorStyle,
          ".MuiChip-label": {
            /*width: "calc(100% - 36px)", maxWidth: "94px"*/ flexGrow: 1,
            flexShrink: 1,
            width: 0,
          },
...
export const EncodingBox: FC<EncodingBoxProps> = function EncodingBox({
  channel,
  chartId,
  tableId,
}) {
  ...
  let updateEncProp = (prop: keyof EncodingItem, value: any) => {
    dispatch(
      dfActions.updateChartEncodingProp({
        chartId,
        channel,
        prop: prop as string,
        value,
  ...
  let fieldMetadata =
    field?.name && activeTable?.metadata[field?.name]
      ? activeTable?.metadata[field?.name]
      : undefined;
  ...
  let dataTypeOpt = [
    <FormLabel
      key={`enc-box-${channel}-data-type-label`}
      sx={{ fontSize: "inherit" }}
      id="data-type-option-radio-buttons-group"
    >
      Data Type
    </FormLabel>,
    <FormControl
  ...
  let stackOpt =
    ...
    (channel == "x" || channel == "y")
      ? [
          <FormLabel
            key={`enc-box-${channel}-stack-label`}
            sx={{ fontSize: "inherit" }}
            id="normalized-option-radio-buttons-group"
          >
            Stack
          </FormLabel>,
          <FormControl
  ...
  let domainItems = field
    ? activeTable?.rows.map((row) => row[field?.name])
    : [];
  domainItems = [...new Set(domainItems)];

  let autoSortEnabled =
    field && fieldMetadata?.type == Type.String && domainItems.length < 200;
        ...
        if (data["status"] == "ok") {
          if (data["token"] == token) {
            ...
            if (candidate["status"] == "ok") {
              let sortRes = {
                values: candidate["content"]["sorted_values"],
  ...
  let sortByOptions = [radioLabel("auto", "auto", `sort-by-auto`)];
  // TODO: check sort options
  if (
    channel == "x" &&
    (fieldMetadata?.type == Type.String || fieldMetadata?.type == Type.Auto)
  ) {
  ...
  let sortByOpt = [
    <FormLabel
      sx={{ fontSize: "inherit" }}
      key={`enc-box-${channel}-sort-label`}
      id="sort-option-radio-buttons-group"
    >
      Sort By
    </FormLabel>,
    <FormControl
    ...
    >
      <RadioGroup
        ...
        sx={{ width: 180 }}
        onChange={(event) => {
          updateEncProp("sortBy", event.target.value);
        }}
      >
        {sortByOptions}
      </RadioGroup>
  ...
  let sortOrderOpt = [
    <FormLabel
      sx={{ fontSize: "inherit" }}
      key={`enc-box-${channel}-sort-order-label`}
      id="sort-option-radio-buttons-group"
    >
      Sort Order
    </FormLabel>,
    ...
    >
      <RadioGroup
        ...
        sx={{ width: 180 }}
        onChange={(event) => {
          updateEncProp("sortOrder", event.target.value);
        }}
      >
  ...
  let colorSchemeOpt =
    channel == "color"
      ? [
          <FormLabel
            sx={{ fontSize: "inherit" }}
            key={`enc-box-${channel}-color-scheme-label`}
            id="scheme-option-radio-buttons-group"
          >
            Color scheme
          </FormLabel>,
          <FormControl
            ...
            fullWidth
            size="small"
            sx={{ textAlign: "initial", fontSize: "12px" }}
          >
            <Select
              ...
              label=""
              sx={{
                "& .MuiSelect-select": { fontSize: "12px", paddingLeft: "6px" },
              }}
              value={encoding.scheme || "default"}
              onChange={(event) => {
                updateEncProp("scheme", event.target.value);
              }}
            >
              <MenuItem value={"default"} key={"color-scheme--1"}>
                <em>default</em>
              </MenuItem>
              {colorSchemeList.map((t, i) => (
                <MenuItem value={t} key={`color-scheme-${i}`}>
                  {t}
                </MenuItem>
              ))}
            </Select>
  ...
  let aggregateDisplay = encoding.aggregate ? (
    <Chip
      key="aggr-display"
      className="encoding-prop-chip"
      sx={{
        backgroundColor: optBackgroundColor,
        width: field == undefined ? "100%" : "auto",
      }}
      onDelete={() => updateEncProp("aggregate", undefined)}
      color="default" //deleteIcon={<RemoveIcon />}
      label={encoding.aggregate == "average" ? "avg" : encoding.aggregate}
      size="small"
    />
  ) : (
    ""
  ...
  let normalizedDisplay = encoding.stack ? (
    <Chip
      ...
      className="encoding-prop-chip" //deleteIcon={<RemoveIcon />}
      color="default"
      sx={{ backgroundColor: optBackgroundColor }}
      label={"⌸"}
      size="small"
      onDelete={() => updateEncProp("stack", undefined)}
    />
  ) : (
    ""
  );
  ...
  conceptGroups.sort((a, b) => {
    if (groupNames.indexOf(a.group) < groupNames.indexOf(b.group)) {
      ...
      return activeTable &&
        activeTable.names.includes(a.field.name) &&
        !activeTable.names.includes(b.field.name)
        ? -1
        : 1;
    }
  });
  ...
  let createConceptInputBox = (
    <Autocomplete
      ...
      renderOption={(props, option) => {
        let renderOption = conceptShelfItems.map((f) => f.name).includes(option)
          ? option
          : `${option}`;
        let otherStyle =
          option == `` ? { color: "darkgray", fontStyle: "italic" } : {};
      ...
      renderInput={(params) => (
        <TextField
          ...
          sx={{
            height: "24px",
            "& .MuiInput-root": { height: "24px", fontSize: "small" },
          }}
        />
      )}
      slotProps={{
        paper: {
          // Use paper instead of popper for styling
          sx: {
            width: "300px",
            maxWidth: "300px",
            "& .MuiAutocomplete-listbox": {
  ...
  const encContent =
    field == undefined
      ? encoding.aggregate == "count"
        ? [aggregateDisplay]
        : [normalizedDisplay, aggregateDisplay, createConceptInputBox]
      : [normalizedDisplay, aggregateDisplay, fieldComponent];
  ...
  let encodingComp = (
    ...
    >
      <Box
        ...
        component="form"
        className="channel-shelf-box encoding-item"
      >
        <Card
          sx={{
            width: "100%",
            boxShadow: editMode
              ? "0 2px 2px 0 rgb(0 0 0 / 20%), 0 2px 2px 0 rgb(0 0 0 / 19%)"
              : "",
          }}
          variant="outlined"
        >
            ...
            >
              ...
              <ArrowDropDownIcon
                sx={{
                  ...
                  right: "0",
                  paddingLeft: "2px",
                  transform: editMode ? "rotate(180deg)" : "",
                }}
                fontSize="inherit"
              />
            </IconButton>
