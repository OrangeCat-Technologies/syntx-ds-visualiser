...
export const TriggerCard: FC<{
  ...
  mini?: boolean;
  sx?: SxProps<Theme>;
}> = function ({ className, trigger, hideFields, mini = false, sx }) {
  let theme = useTheme();
  ...
  if (trigger.instruction == "" && encFields.length > 0) {
    ...
    (trigger.instruction != "" &&
      trigger.instruction.length <=
        trigger.displayInstruction.replace(/\*\*/g, "").length)
  ) {
        ...
        return (
          <Chip
            ...
            sx={{
              ...
              bgcolor: isRequired
                ? "rgba(25, 118, 210, 0.2)"
                : "rgba(25, 118, 210, 0.1)", // darker blue for required
              color: "rgba(0, 0, 0, 0.7)",
              "& .MuiChip-label": {
                pl: "4px",
                pr: "6px",
...
export const EncodingShelfCard: FC<EncodingShelfCardProps> = function ({
  chartId,
}) {
  ...
  let trigger =
    chart.source == "trigger"
      ? tables.find((t) => t.derive?.trigger?.chart?.id == chartId)?.derive
          ?.trigger
      : undefined;
  ...
  // Add this state
  const [userSelectedActionTableIds, setUserSelectedActionTableIds] = useState<
    string[]
  >([]);
  ...
  const currentState = chartState[chartId] || {
    ideas: [],
    thinkingBuffer: "",
    isLoading: false,
  };
  ...
  const setIdeas = (
    ...
    setChartState((prev) => ({
      ...prev,
      [chartId]: {
        ...(prev[chartId] || { thinkingBuffer: "", isLoading: false }),
        ideas,
      },
    }));
  };
  ...
  const setThinkingBuffer = (thinkingBuffer: string) => {
    setChartState((prev) => ({
      ...prev,
      [chartId]: {
        ...(prev[chartId] || { ideas: [], isLoading: false }),
        thinkingBuffer,
      },
    }));
  };
  ...
  const setIsLoadingIdeas = (isLoading: boolean) => {
    setChartState((prev) => ({
      ...prev,
      [chartId]: {
        ...(prev[chartId] || { ideas: [], thinkingBuffer: "" }),
        isLoading,
      },
    }));
  };
  ...
  let encodingBoxGroups = Object.entries(ChannelGroups)
    ...
    .map(([group, channelList]) => {
      let component = (
        <Box key={`encoding-group-box-${group}`}>
          <Typography
          ...
          {channelList
            .filter((channel) => Object.keys(encodingMap).includes(channel))
            .map((channel) => (
              <EncodingBox
                key={`shelf-${channel}`}
  ...
  let activeFields = Object.values(encodingMap)
    .map((enc) => enc.fieldID)
    .filter(
      (fieldId) =>
  ...
  let deriveNewData = (
    ...
    let fieldNamesStr = activeFields
      .map((f) => f.name)
      .reduce(
    ...
    if (currentTable.derive?.dialog && !currentTable.anchored) {
      ...
      let startNewDialog =
        !sourceTableIds.every((id) => actionTableIds.includes(id)) ||
        ...
        if (data.results.length > 0) {
          if (data["token"] == token) {
            ...
            if (candidates.length == 0) {
              ...
              let candidateTable = createDictTable(candidateTableId, rows, {
                code: code,
  ...
  const w: any = (a: any[], b: any[]) =>
    a.length ? [a[0], ...w(b, a.slice(1))] : b;

  let formulateInputBox = (
    <Box
      key="text-input-boxes"
      sx={{
        display: "flex",
        flexDirection: "row",
        flex: 1,
    ...
    >
      <TextField
        ...
        label=""
        placeholder={
          ["Auto"].includes(chart.chartType)
            ? isChartAvailable
              ? "what do you want to visualize?"
              : " ✏️ what do you want to visualize?"
            : isChartAvailable
            ? "formulate data"
            : " ✏️  formulate data"
        }
          ...
          <Tooltip
            title={
              <Typography sx={{ fontSize: 11 }}>
                formulate and override{" "}
                <TableRowsIcon sx={{ fontSize: 10, marginBottom: "-1px" }} />
  ...
  let channelComponent = (
    ...
    >
      {existMultiplePossibleBaseTables && (
        <UserActionTableSelector
      ...
      <Box key="mark-selector-box" sx={{ flex: "0 0 auto" }}>
        <FormControl
          sx={{ m: 1, minWidth: 120, width: "100%", margin: "0px 0" }}
          size="small"
        >
          <Select
            variant="standard"
            ...
            renderValue={(value: string) => {
              ...
              return (
                <div style={{ display: "flex", padding: "0px 0px 0px 4px" }}>
                  ...
                  <ListItemText
                    sx={{ marginLeft: "2px", whiteSpace: "initial" }}
    ...
    <>
      ...
      >
        ...
        {ideateMode ? (
            ...
            <Tooltip title={`get ideas for visualization`}>
              <span>
                <Button
                  ...
                  startIcon={
                    isLoadingIdeas ? undefined : (
                      <LightbulbOutlinedIcon sx={{ fontSize: 10 }} />
                    )
                  }
                  sx={{
                    fontSize: 12,
          ...
          <Box sx={{ padding: 1 }}>{channelComponent}</Box>
        )}
      </Card>
      <Dialog
        open={devMessageOpen}
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>
            ...
            <Typography
              component="a"
              href="https://github.com/OrangeCat-Technologies/syntx-ds-visualiser"
              target="_blank"
