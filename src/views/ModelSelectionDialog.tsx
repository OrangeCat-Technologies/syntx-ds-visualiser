...
export const ModelSelectionButton: React.FC<{}> = ({}) => {
  ...
  const testedModels = useSelector(
    (state: DataFormulatorState) => state.testedModels
  );
  ...
  let getStatus = (id: string | undefined) => {
    return id != undefined
      ? testedModels.find((t) => t.id == id)?.status || "unknown"
      : "unknown";
  };

  // Helper functions for slot management
  const [tempSelectedModelId, setTempSelectedModelId] = useState<
    string | undefined
  >(selectedModelId);
      ...
      try {
        ...
        data.forEach((modelConfig: any) => {
          ...
          if (
            provider &&
            model &&
            !modelsByProvider[provider].includes(model)
          ) {
  ...
  let modelExists = models.some(
    (m) =>
      m.endpoint == newEndpoint &&
      m.model == newModel &&
      m.api_base == newApiBase &&
      m.api_key == newApiKey &&
      m.api_version == newApiVersion
  );
    ...
    >
        ...
        <Autocomplete
          ...
          onChange={(event: any, newValue: string | null) => {
            ...
            if (
              newModel == "" &&
              newValue == "openai" &&
              providerModelOptions.openai.length > 0
            ) {
      ...
      <TableCell align="right">
        <TextField
          ...
          value={newApiBase}
          onChange={(event: any) => {
            setNewApiBase(event.target.value);
          }}
          autoComplete="off"
        />
      </TableCell>
      <TableCell align="right">
        <TextField
          ...
          value={newApiVersion}
          onChange={(event: any) => {
            setNewApiVersion(event.target.value);
          }}
          autoComplete="off"
          placeholder="optional"
        />
      ...
      >
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ fontWeight: "bold", width: "160px" }} align="left">
              Model
            </TableCell>
            <TableCell sx={{ fontWeight: "bold", width: "200px" }} align="left">
              API Base
            </TableCell>
            <TableCell sx={{ fontWeight: "bold", width: "120px" }} align="left">
              API Version
            </TableCell>
        ...
        <TableBody>
          {models.map((model) => {
            ...
            if (status == "unknown") {
              ...
              const rawMessage =
                testedModels.find((t) => t.id == model.id)?.message ||
                "Unknown error";
              message = `Error: ${decodeHtmlEntities(
                rawMessage
              )}. Click to retest.`;
            }

            const borderStyle = ["error"].includes(status)
              ? "1px dashed lightgray"
              : undefined;
            const noBorderStyle = ["error"].includes(status)
              ? "none"
              : undefined;
            const disabledStyle =
              status != "ok" ? { cursor: "default", opacity: 0.5 } : undefined;
            ...
            return (
              <React.Fragment key={`${model.id}`}>
                ...
                >
                  <TableCell
                    align="left"
                    sx={{ borderBottom: noBorderStyle, ...disabledStyle }}
                  >
                    <Typography
                      variant="body2"
                  ...
                  <TableCell
                    component="th"
                    scope="row"
                    sx={{ borderBottom: borderStyle, ...disabledStyle }}
                  >
                    {model.api_key ? (
                      showKeys ? (
                        <Typography
  ...
  let modelNotReady =
    tempSelectedModelId == undefined || getStatus(tempSelectedModelId) !== "ok";

  let tempModel = models.find((m) => m.id == tempSelectedModelId);
  let tempModelName = tempModel
    ? `${tempModel.endpoint}/${tempModel.model}`
    : "Please select a model";
  let selectedModelName =
    models.find((m) => m.id == selectedModelId)?.model || "Unselected";
    ...
    <>
      <Tooltip title="Select a model">
        <Button
          ...
          variant="text"
          color={modelNotReady ? "warning" : "primary"}
          onClick={() => {
      ...
      >
          ...
          >
            <Box>
              <Typography
                ...
                sx={{ lineHeight: 1.6 }}
              >
                • Models with strong code generation capabilities (e.g.,{" "}
                <code
                  style={{
              ...
              >
                • Model configuration based on LiteLLM.{" "}
                <a
        ...
        <DialogActions>
          {!serverConfig.DISABLE_DISPLAY_KEYS && (
            <Button
              sx={{ marginRight: "auto" }}
              endIcon={showKeys ? <VisibilityOffIcon /> : <VisibilityIcon />}
              onClick={() => {
