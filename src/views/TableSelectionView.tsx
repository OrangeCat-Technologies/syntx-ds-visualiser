...
export const DatasetSelectionView: React.FC<DatasetSelectionViewProps> =
  function DatasetSelectionView({ datasets, handleSelectDataset, hideRowNum }) {
              ...
              return (
                <Box key={j}>
                  ...
                  >
                    ...
                    {Object.keys(t.rows[0]).length} columns
                    {hideRowNum ? "" : ` ⨉ ${t.rows.length} rows`})
                  </Typography>
                  {content}
                </Box>
              );
              ...
              <Box key={i}>
                ...
                >
                  <Typography sx={{ fontSize: 12 }}>
                    ...
                    <Typography
                      variant="caption"
                      sx={{ color: "primary.light", fontSize: 10, mx: 0.5 }}
                    >
                      [from {dataset.source}]
                    </Typography>
                  </Typography>
...
export const DatasetSelectionDialog: React.FC<{ buttonElement: any }> =
  function DatasetSelectionDialog({ buttonElement }) {
    const [datasetPreviews, setDatasetPreviews] = React.useState<
      DatasetMetadata[]
    >([]);
    ...
    React.useEffect(() => {
      ...
      fetch(`${getUrls().EXAMPLE_DATASETS}`)
        ...
        .then((result) => {
          let datasets: DatasetMetadata[] = result
            .map((info: any) => {
...
export const TableUploadDialog: React.FC<TableUploadDialogProps> = ({
  buttonElement,
  disabled,
  onOpen,
  fileInputRef,
          ...
          if (file.size > MAX_FILE_SIZE) {
            dispatch(
              dfActions.addMessages({
                ...
                value: `File ${file.name} is too large (${(
                  file.size /
                  (1024 * 1024)
          ...
          title={
            serverConfig.DISABLE_FILE_UPLOAD ? (
              <Typography sx={{ fontSize: "11px" }}>
                Install DS Visualiser locally to enable file upload. <br />
                Link:{" "}
                <Link
                ...
                >
                  https://github.com/OrangeCat-Technologies/syntx-ds-visualiser
                </Link>
              </Typography>
            ) : (
              ""
...
export const TableURLDialog: React.FC<TableURLDialogProps> = ({
  buttonElement,
  disabled,
}) => {
  ...
  let hasValidSuffix =
    tableURL.endsWith(".csv") ||
    tableURL.endsWith(".tsv") ||
    tableURL.endsWith(".json");

  let dialog = (
    <Dialog
      key="table-url-dialog"
      onClose={() => {
        setDialogOpen(false);
      }}
      open={dialogOpen}
    ...
    >
      ...
      >
        <TextField
          error={tableURL != "" && !hasValidSuffix}
          autoFocus
          placeholder="Please enter URL of the dataset"
          helperText={
            hasValidSuffix
              ? ""
              : "the url should links to a csv, tsv or json file"
          }
          sx={{ marginBottom: 1 }}
          size="small"
          value={tableURL}
          onChange={(event) => {
            setTableURL(event.target.value.trim());
          }}
          id="dataset-url"
          label="data url"
          variant="outlined"
          ...
          <Box sx={{ flex: 1, display: "flex", flexDirection: "column" }}>
            ...
            {isOverSizeLimit && (
              ...
              >
                <Typography
                  variant="caption"
                  sx={{ flex: 1, color: "error.main", fontWeight: 500 }}
                >
                  ⚠️ Content exceeds{" "}
                  {(MAX_CONTENT_SIZE / (1024 * 1024)).toFixed(0)}MB size limit.
                  Current size:{" "}
                  {(new Blob([tableContent]).size / (1024 * 1024)).toFixed(2)}
                  MB. Please use the DATABASE option for large datasets.
                </Typography>
              </Box>
            )}
            ...
            {isLargeContent && !isOverSizeLimit && (
              ...
              >
                <Typography variant="caption" sx={{ flex: 1 }}>
                  ...
                  {Math.round(tableContent.length / 1000)}KB).
                  {showFullContent
                    ? "Showing full content (may be slow)"
                    : "Showing preview for performance"}
                </Typography>
                <Button
                  size="small"
                  variant="outlined"
            ...
            <TextField
              ...
              sx={{
                ...
                "& .MuiInputBase-input": {
                  ...
                  maxHeight:
                    isLargeContent && !showFullContent ? "300px" : "400px",
                  overflow: "auto",
                },
              }}
              id="upload content"
              ...
              maxRows={
                isLargeContent && !showFullContent ? MAX_DISPLAY_LINES : 25
              } // Dynamic max rows
