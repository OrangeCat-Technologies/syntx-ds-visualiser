...
export const FreeDataViewFC: FC<FreeDataViewProps> = function DataView() {
  ...
  let renderTableBody = (targetTable: DictTable | undefined) => {
    ...
    const calculateColumnWidth = (name: string) => {
      ...
      const minWidth =
        Math.max(60, contentLength * 8 > 200 ? 200 : contentLength * 8) + 50; // 8px per character with 50px padding
      const width = minWidth;

      return { minWidth, width };
    };

    let colDefs = targetTable
      ? targetTable.names.map((name, i) => {
          ...
          return {
            ...
            source:
              conceptShelfItems.find((f) => f.name == name)?.source ||
              "original",
          };
        })
      : [];
      ...
      colDefs = [
        {
          ...
          width: 40,
          format: (value: any) => (
            <Typography fontSize="inherit" color="rgba(0,0,0,0.65)">
              {value}
            </Typography>
          ),
      ...
      <Fade in={true} timeout={600} key={targetTable?.id}>
        <Box sx={{ height: "calc(100% - 28px)" }}>
          <SelectableDataGrid
            ...
            columnDefs={colDefs}
            rowCount={
              targetTable?.virtual?.rowCount || targetTable?.rows.length || 0
            }
  ...
  let genTableLink = (t: DictTable) => (
    <Link
      ...
      onClick={() => {
        dispatch(dfActions.setFocusedTable(t.id));
      }}
    >
      <Typography
        sx={{
          fontWeight: t.id === focusedTableId ? "bold" : "inherit",
          fontSize: "inherit",
        }}
        component="span"
      >
  ...
  return (
    ...
    >
      <Box sx={{ display: "flex", alignItems: "center" }}>
        <Breadcrumbs
          sx={{ fontSize: "12px", margin: "4px 12px" }}
          separator="›"
