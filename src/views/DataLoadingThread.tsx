...
export const DataPreviewBox: React.FC<{ sx?: SxProps }> = ({ sx }) => {
  ...
  let selectedBlock =
    dataCleanBlocks.find(
      (block) => block.id === focusedDataCleanBlockId?.blockId
    ) || dataCleanBlocks[dataCleanBlocks.length - 1];
  let selectedTable = focusedDataCleanBlockId
    ? selectedBlock?.items?.[focusedDataCleanBlockId.itemId]
    : undefined;
  ...
  if (selectedTable.content.type === "csv" && selectedTable.content.value) {
    const suggestedName =
      selectedTable.name ||
      generateDefaultName(selectedTable.content.value.slice(0, 96));
    const tableComponent = createTableFromText(
      suggestedName,
  ...
  if (selectedTable.content.type === "web_url") {
    return (
      ...
      >
        <Typography
          variant="body2"
          sx={{ fontSize: "12px", color: "text.primary", fontWeight: 500 }}
        >
          Data URL
        </Typography>
        <Typography
...
export const DataLoadingInputBox = React.forwardRef<
  (() => void) | null,
  {
    maxLines?: number;
    onStreamingContentUpdate?: (content: string) => void;
    abortControllerRef?: React.MutableRefObject<AbortController | null>;
  }
>(({ maxLines = 4, onStreamingContentUpdate, abortControllerRef }, ref) => {
  const dispatch = useDispatch<AppDispatch>();
  ...
  let selectedBlock = focusedDataCleanBlockId
    ? dataCleanBlocks.find(
        (block) => block.id === focusedDataCleanBlockId.blockId
      )
    : undefined;
  ...
  const sampleTasks = [
    {
      ...
      fullText:
        "extract the top repos information from https://github.com/microsoft?q=&type=all&language=&sort=stargazers",
  ...
  const sendRequest = (promptToUse: string, imagesToUse: string[]) => {
    ...
    let prompt_to_send =
      ...
      (hasImageData
        ...
        : "let's generate some interesting data");
    ...
    const payload: any = {
      ...
      artifacts: [
        ...images_to_send.map((image) => ({
          type: "image_url",
          content: image,
        })),
        ...extractedUrlsFromPrompt.map((url) => ({
          type: "web_url",
          content: url,
        })),
      ],
        ...
        try {
          ...
          if (
            ...
            const newBlock: DataCleanBlock = {
              ...
              dialogItem:
                updatedDialog.length > 0
                  ? updatedDialog[updatedDialog.length - 1]
                  : undefined,
            };
            ...
            dispatch(
              dfActions.setFocusedDataCleanBlockId({
                blockId: newBlock.id,
                itemId: 0,
              })
            );
            ...
            dispatch(
              dfActions.addMessages({
                ...
                value:
                  finalResult?.content ||
                  "Unable to extract tables from response",
              })
            );
    ...
    >
      ...
      <Box sx={{ flex: 1, position: "relative" }}>
        ...
        {!existOutputBlocks && !cleanInProgress && (
          <Box sx={{ mt: 2, mb: 1 }}>
            <Typography
              sx={{ fontSize: "11px", color: "text.secondary", mb: 1 }}
            >
              examples
            </Typography>
            <Box
...
export const createOrderedThreadBlocks = (
  ...
  const getBlockPath = (blockId: string): DataCleanBlock[] => {
    ...
    while (currentBlock) {
      ...
      currentBlock = dataCleanBlocks.find(
        (b) => b.id === currentBlock?.derive.sourceId
      );
    }
  ...
  const getLeafBlocks = (): DataCleanBlock[] => {
    return dataCleanBlocks.filter((block) => {
      ...
      return !dataCleanBlocks.some(
        (otherBlock) => otherBlock.derive.sourceId === block.id
      );
    });
  };

  // Get blocks that should be displayed in a thread (avoiding repetition)
  const getThreadBlocks = (
    leafBlock: DataCleanBlock,
...
export const SingleDataCleanThreadView: React.FC<{
  thread: ThreadBlock;
  sx?: SxProps;
}> = ({ thread, sx }) => {
  ...
  let isThreadFocused = blocks.some(
    (block) => block.id === focusedDataCleanBlockId?.blockId
  );
      ...
      {blocks.map((block, blockIndex) => {
        ...
        return (
          ...
          >
            ...
            <Box
              sx={{
                borderLeft: isThreadFocused ? "3px solid" : "1px dashed",
                borderColor: isThreadFocused
                  ? "primary.light"
                  : " rgba(0, 0, 0, 0.3)",
                py: blockIndex === 0 ? 0.5 : 1.5,
                px: 1,
                display: "flex",
                alignItems: "center",
                ml: isThreadFocused ? "7px" : "8px",
              }}
            ></Box>
            ...
            >
              <PrecisionManufacturingIcon
                sx={{ ml: 1, color: "darkgray", width: "14px", height: "14px" }}
              />
              <Box sx={{ margin: "4px 8px 4px 2px" }}>
                ...
                {block.derive.artifacts &&
                  block.derive.artifacts.filter((a) => a.type === "image_url")
                    .length > 0 && (
                    <Box
                      sx={{
                        display: "flex",
            ...
            <Box sx={{ display: "flex", alignItems: "center" }}>
              <SouthIcon
                color={isThreadFocused ? "primary" : "secondary"}
            ...
            {block.items?.map((table, itemId) => {
              const isItemSelected =
                block.id === focusedDataCleanBlockId?.blockId &&
                itemId === focusedDataCleanBlockId.itemId;
              return (
                <Card
                  variant="outlined"
                  sx={{
                ...
                >
                  <Box sx={{ margin: "0px", display: "flex", py: 0.5 }}>
                    <Stack
                      ...
                      gap={"2px"}
                    >
                      {table.content.type === "csv" && (
                        <TableIcon sx={{ color: "darkgray", fontSize: 14 }} />
                      )}
                      {table.content.type === "image_url" && (
                        <LinkIcon sx={{ color: "darkgray", fontSize: 14 }} />
                      )}
                      {table.content.type === "web_url" && (
                        <LinkIcon sx={{ color: "darkgray", fontSize: 14 }} />
                      )}
                      <Typography
                        fontSize="inherit"
                        ...
                        <Tooltip title="delete table">
                          <IconButton
                            ...
                            onClick={(event) => {
                              ...
                              dispatch(
                                dfActions.removeDataCleanBlocks({
                                  blockIds: [block.id],
                                })
                              );
                            }}
...
export const DataLoadingThread: React.FC = () => {
  ...
  let threadDisplay = (
    ...
    >
      {/* Render each thread */}
      {threads.map((thread) => (
        <SingleDataCleanThreadView
          key={`thread-${thread.threadIndex}`}
          thread={thread}
        />
      ))}
    </Box>
