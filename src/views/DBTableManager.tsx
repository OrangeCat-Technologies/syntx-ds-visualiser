...
export const DBTableSelectionDialog: React.FC<{
  ...
  const handleDropTable = async (tableName: string) => {
    if (tables.some((t) => t.id === tableName)) {
      if (
        !confirm(
          `Are you sure you want to delete ${tableName}? \n ${tableName} is currently loaded into the DS Visualiser and will be removed from the database.`
        )
      )
        return;
    }
    ...
    <>
      {buttonElement && (
          ...
          title={
            serverConfig.DISABLE_DATABASE ? (
              <Typography sx={{ fontSize: "11px" }}>
                Install DS Visualiser locally to use database. <br />
                Link:{" "}
                <Link
        ...
        >
          <span
            style={{
              cursor: serverConfig.DISABLE_DATABASE ? "help" : "pointer",
            }}
          >
            <Button
              variant="text"
              sx={{ fontSize: "inherit", gap: 1 }}
...
export const DataLoaderForm: React.FC<{
  ...
  let tableMetadataBox = [
              ...
              return [
                <TableRow
                  ...
                  sx={{
                    ...
                    backgroundColor: selectedTables.has(tableName)
                      ? "action.selected"
                      : "inherit",
                    "&:hover": {
                      backgroundColor: selectedTables.has(tableName)
    ...
    mode === "view tables" && Object.keys(tableMetadata).length > 0 && (
            ...
            Promise.all(importPromises)
              .then((results) => {
                ...
                if (errors.length === 0) {
                  ...
                  onFinish(
                    "success",
                    `Successfully imported ${tablesToImport.length} table(s)`,
                    tablesToImport
                  );
                } else {
                  onFinish(
                    "error",
                    `Failed to import some tables: ${errors
                      .map((e) => e.error)
                      .join(", ")}`
                  );
                }
