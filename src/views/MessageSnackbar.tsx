...
export function MessageSnackbar() {
  ...
  const displayedMessageIdx = useSelector(
    (state: DataFormulatorState) => state.displayedMessageIdx
  );
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    const key = `${msg.value}|${msg.detail || ""}|${msg.code || ""}|${
      msg.type
    }`;
    ...
    const lastKey = lastGroup
      ? `${lastGroup.value}|${lastGroup.detail || ""}|${lastGroup.code || ""}|${
          lastGroup.type
        }`
      : null;
      ...
      >
        ...
        >
          ...
          <Box sx={{ display: "flex", alignItems: "center", px: 1.5 }}>
            <Typography
              variant="subtitle1"
              sx={{ fontSize: 12, flexGrow: 1, color: "text.secondary" }}
            >
          ...
          >
            {messages.length == 0 && (
              <Typography
                fontSize={12}
                component="span"
                sx={{ margin: "auto", m: 1, opacity: 0.7, fontStyle: "italic" }}
              >
            ...
            {groupedMessages.map((msg, index) => (
              ...
              >
                <Box
                  key={`${msg.originalIndex}-${msg.count}`}
                ...
                >
                  ...
                  <Typography fontSize={11} component="span">
                    [{formatTimestamp(msg.timestamp)}] ({msg.component}) -{" "}
                    {msg.value}
                  </Typography>
                  {msg.count > 1 && (
                    <Chip
                  ...
                  >
                    ...
                    {msg.code && (
                      <>
                        <Divider
                          textAlign="left"
                          sx={{ my: 1, fontSize: 12, opacity: 0.7 }}
                        >
                          [generated code]
                        </Divider>
                        <Typography
