...
export const DerivedDataDialog: FC<DerivedDataDialogProps> =
  function DerivedDataDialog({
    chart,
    candidateTables,
    open,
    handleCloseDialog,
    ...
    let body = (
      ...
      >
        {candidateTables.map((table, idx) => {
          ...
          let assembledChart: any = assembleVegaChart(
            chart.chartType,
            chart.encodingMap,
            conceptShelfItems,
            extTable,
          ...
          const element = (
            <Box
              className="vega-thumbnail-no-hover"
              id={id}
              ...
              if (result.view.container()?.getElementsByTagName("canvas")) {
                ...
                if (comp) {
                  ...
                  if (width > 240 || height > 180) {
                    ...
                    comp?.setAttribute(
                      "style",
                      `max-width: 240px; max-height: 180px; width: ${Math.round(
                        fixedWidth
          ...
          let simpleTableView = (t: DictTable) => {
            let colDefs = t.names.map((name) => {
              return {
                ...
                source: conceptShelfItems.find((f) => f.name == name)?.source,
              };
            });
            return (
              <Box
            ...
            >
              ...
              >
                <FormControlLabel
                  ...
                  value={idx}
                  control={<Radio checked={selectionIdx == idx} />}
                  label={
                    <Typography
                      color={idx == selectionIdx ? "primary" : "inherit"}
                    >{`candidate-${idx + 1} (${
                      candidateTables[idx].id
                    })`}</Typography>
                  }
                />
                <Box
              ...
              <ButtonGroup size="small" sx={{ margin: "auto" }}>
                ...
                >
                  ...
                  <Typography
                    component="span"
                    fontSize="inherit"
                    sx={{ margin: "0px 2px", padding: "0px 2px" }}
                  >{`candidate ${selectionIdx + 1} (${
                    candidateTables[selectionIdx].id
                  })`}</Typography>{" "}
                  as the result
                </Button>
              </ButtonGroup>
    ...
    return (
      ...
      >
        <DialogTitle>
          <Typography>Derived Data Candidates</Typography>
        </DialogTitle>
