...
export let ChartElementFC: FC<{
  ...
  boxWidth?: number;
  boxHeight?: number;
}> = function ({ chart, tableRows, tableMetadata, boxWidth, boxHeight }) {
  ...
  const id = `chart-thumbnail-${chart.id}-${(Math.random() + 1)
    .toString(36)
    .substring(7)}`;
  const element = (
    <Box
  ...
  embed("#" + id, assembledChart, { actions: false, renderer: "canvas" })
      ...
      if (result.view.container()?.getElementsByTagName("canvas")) {
        ...
        if (comp) {
          ...
          if (width > WIDTH || height > HEIGHT) {
            ...
            comp?.setAttribute(
              "style",
              `max-width: ${WIDTH}px; max-height: ${HEIGHT}px; width: ${Math.round(
                fixedWidth
              )}px; height: ${Math.round(fixedWidth / ratio)}px; `
            );
          }
        } else {
...
export const EncodingShelfThread: FC<EncodingShelfThreadProps> = function ({
  chartId,
}) {
  ...
  let activeTableThread = [
    ...getTriggers(t, tables).map((tr) => tr.tableId),
    chart.tableRef,
  ];

  const dispatch = useDispatch<AppDispatch>();

  const interleaveArrays: any = (a: any[], b: any[], spaceElement?: any) =>
    a.length
      ? [
          a[0],
          spaceElement || "",
          ...interleaveArrays(b, a.slice(1), spaceElement),
        ]
      : b;
  ...
  let buildTableCard = (tableId: string) => {
    ...
    return (
      <div key={`${tableId}-table-list-item`} className="table-list-item">
        <Button
          variant="text"
  ...
  let leafTable = tables.find(
    (t) => t.id == activeTableThread[activeTableThread.length - 1]
  ) as DictTable;
  ...
  let instructionCards = triggers.map((trigger, i) => {
    let extractActiveFields = (t: Trigger) => {
      ...
      return Array.from(Object.values(encodingMap))
        .map((enc: EncodingItem) => enc.fieldID)
        .filter((x) => x != undefined);
    };
    let previousActiveFields = new Set(
      i == 0 ? [] : extractActiveFields(triggers[i - 1])
    );
    ...
    return (
      ...
      >
        <Box
          sx={{ ml: "8px", height: "4px", borderLeft: "1px solid lightgray" }}
        ></Box>
        ...
        />
        <Box
          sx={{ ml: "8px", height: "4px", borderLeft: "1px solid darkgray" }}
        ></Box>
      </Box>
    );
  });
  ...
  if (chartTrigger) {
    let resultTable = tables.find(
      (t) => t.id == chartTrigger.resultTableId
    ) as DictTable;
    let leafUserCharts = allCharts
      .filter((c) => c.tableRef == resultTable.id)
    ...
    let endChartCards = leafUserCharts.map((c) => {
      return (
        ...
        >
          <ChartElementFC
            chart={c}
            tableRows={resultTable.rows.slice(0, 100)}
            tableMetadata={resultTable.metadata}
            boxWidth={200}
            boxHeight={160}
          />
        </Card>
  ...
  const encodingShelf = (
    <Box
      className="encoding-shelf-compact"
      sx={{
        height: "100%",
        width: 236,
        overflowY: "auto",
    ...
    >
      {[
        <Box key="encoding-shelf" sx={{ display: "flex" }}>
  ...
  return (
    ...
    >
      ...
      >
        <Tooltip
          placement="left"
          title={collapseEditor ? "open editor" : "hide editor"}
        >
          <Button
            color="primary"
            sx={{ width: 18, minWidth: 18, p: 0 }}
            onClick={() => {
              setCollapseEditor(!collapseEditor);
            }}
          >
            {collapseEditor ? (
