# Set up a local DS Visualiser development environment
...
- **Configure environment variable (optional)s**
    - copy `api-keys.env.example` to `api-keys.env` and add your API keys.
        ...
        - this helps DS Visualiser to automatically load the API keys when you run the app, so you don't need to set the API keys in the app UI.

    - set `.env` to configure server properties:
        - copy `.env.template` to `.env`
        - configure settings as needed:
            - DISABLE_DISPLAY_KEYS: if true, API keys will not be shown in the frontend
            - EXEC_PYTHON_IN_SUBPROCESS: if true, Python code runs in a subprocess (safer but slower), you may consider setting it true when you are hosting DS Visualiser for others
...
- **Test the artifact**
    ...
    Once installed, you can run DS Visualiser with:
    ```bash
    data_formulator
    ```
    or 
...
When deploying DS Visualiser to production, please be aware of the following security considerations:

### Database Storage Security

1. **Local DuckDB Files**: When database functionality is enabled (default), DS Visualiser stores DuckDB database files locally on the server. These files contain user data and are stored in the system's temporary directory or a configured `LOCAL_DB_DIR`.
...
3. **Data Persistence**: User data processed through DS Visualiser may be temporarily stored in these local DuckDB files, which could be a security risk in multi-tenant environments.
