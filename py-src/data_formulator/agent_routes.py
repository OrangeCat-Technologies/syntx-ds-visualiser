            ...
            try:
                ...
                response = client.get_completion(
                    messages=[
                        ...
                        {
                            "role": "user",
                            "content": "Respond 'I can hear you.' if you can hear me.",
                        },
                    ]
                )
        ...
        try:
            response = client.get_completion(
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": "Respond 'I can hear you.' if you can hear me. Do not say anything other than 'I can hear you.'",
                    },
                ]
            )
            ...
            if "I can hear you." in response.choices[0].message.content:
                result = {"model": content["model"], "status": "ok", "message": ""}
        except Exception as e:
            print(f"Error: {e}")
...
def derive_concept_request():
    if request.is_json:
        ...
        candidates = agent.run(
            content["input_data"],
            [f["name"] for f in content["input_fields"]],
            content["output_name"],
            content["description"],
        )
...
def derive_data():
    if request.is_json:
        ...
        language = content.get(
            "language", "python"
        )  # whether to use sql or python, default to python

        max_repair_attempts = (
            content["max_repair_attempts"] if "max_repair_attempts" in content else 1
        )
        ...
        conn = (
            db_manager.get_connection(session["session_id"])
            if language == "sql"
            else None
        )
        ...
        if mode == "recommendation":
            ...
            agent = (
                SQLDataRecAgent(
                    client=client, conn=conn, agent_coding_rules=agent_coding_rules
                )
                if language == "sql"
                else PythonDataRecAgent(
                    client=client,
                    exec_python_in_subprocess=current_app.config["CLI_ARGS"][
                        "exec_python_in_subprocess"
                    ],
                    agent_coding_rules=agent_coding_rules,
                )
            )
            results = agent.run(
                input_tables, instruction, n=1, prev_messages=prev_messages
            )
        else:
            agent = (
                ...
                if language == "sql"
                else PythonDataTransformationAgent(
                    client=client,
                    exec_python_in_subprocess=current_app.config["CLI_ARGS"][
                        "exec_python_in_subprocess"
                    ],
            ...
            results = agent.run(
                input_tables, instruction, chart_type, chart_encodings, prev_messages
            )

        repair_attempts = 0
        while (
            results[0]["status"] == "error" and repair_attempts < max_repair_attempts
        ):  # try up to n times
            ...
            if mode == "recommendation":
                results = agent.followup(
                    input_tables, prev_dialog, [], new_instruction, n=1
                )
        ...
        if request.is_json:
            ...
            try:
                for result in run_exploration_flow_streaming(
                    ...
                    response_data = {"token": token, "status": "ok", "result": result}
...
def query_completion():
    if request.is_json:
        ...
        response = flask.jsonify(
            {"token": "", "status": "ok", "reasoning": reasoning, "query": query}
        )
    else:
        response = flask.jsonify(
            {
                "token": "",
                "status": "error",
                "reasoning": "unable to complete query",
                "query": "",
            }
        )
    ...
    def generate():
        if request.is_json:
            ...
            try:
                ...
                logger.error(e)
                error_data = {"content": "unable to process report generation request"}
                yield "error: " + json.dumps(error_data) + "\n"
        else:
            error_data = {"content": "Invalid request format"}
            yield "error: " + json.dumps(error_data) + "\n"
