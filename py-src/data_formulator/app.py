...
app.config["CLI_ARGS"] = {
    "exec_python_in_subprocess": os.environ.get(
        "EXEC_PYTHON_IN_SUBPROCESS", "false"
    ).lower()
    == "true",
    "disable_display_keys": os.environ.get("DISABLE_DISPLAY_KEYS", "false").lower()
    == "true",
    "disable_database": os.environ.get("DISABLE_DATABASE", "false").lower() == "true",
    "disable_file_upload": os.environ.get("DISABLE_FILE_UPLOAD", "false").lower()
    == "true",
    "project_front_page": os.environ.get("PROJECT_FRONT_PAGE", "false").lower()
    == "true",
}
...
def page_not_found(e):
    ...
    )  #'Hello 404!' #send_from_directory(app.static_folder, "index.html")
...
def get_session_id():
    ...
    if database_disabled:
        ...
        if current_session_id is None:
            ...
            logger.info(
                f"Generated session ID for disabled database: {current_session_id}"
            )
        else:
            logger.info(
                f"Using provided session ID for disabled database: {current_session_id}"
            )

        return flask.jsonify({"status": "ok", "session_id": current_session_id})
    else:
        ...
        return flask.jsonify({"status": "ok", "session_id": session["session_id"]})
...
def database_disabled_fallback(path):
    ...
    if app.config["CLI_ARGS"]["disable_database"]:
        ...
        return flask.jsonify(
            {"status": "error", "message": "Table routes are not available"}
        ), 404
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=5000, help="The port number you want to use"
    )
    parser.add_argument(
        "--exec-python-in-subprocess",
        action="store_true",
        default=False,
