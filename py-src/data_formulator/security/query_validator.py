...
def normalize_query(query: str) -> str:
    ...
    query_normalized = re.sub(
        r"/\*.*?\*/", "", query_normalized, flags=re.DOTALL
    )  # Multi-line comments
        ...
        for category, patterns in dangerous_patterns.items():
            for pattern in patterns:
                if re.search(pattern, query_normalized, re.IGNORECASE):
                    return (
                        False,
                        f"Dangerous {category.replace('_', ' ')} detected: {pattern}",
                    )

        # Check for file paths in string literals
        string_literals = re.findall(r"'([^']*)'", query_normalized) + re.findall(
            r'"([^"]*)"', query_normalized
        )
        for literal in string_literals:
            for pattern in dangerous_patterns["file_path_patterns"]:
                if re.search(pattern, literal, re.IGNORECASE):
                    return (
                        False,
                        f"Dangerous file path detected in string literal: {literal}",
                    )
