...
def create_chart_spec_from_data(
    transformed_data: Dict[str, Any], chart_type: str, chart_encodings: Dict[str, str]
) -> str:
    """
...
def run_exploration_flow_streaming(
    ...
    exploration_agent = ExplorationAgent(
        client, db_conn=db_conn, agent_exploration_rules=agent_exploration_rules
    )

    # rec agent for data transformation
    if language == "sql":
        rec_agent = SQLDataRecAgent(
            client=client, conn=db_conn, agent_coding_rules=agent_coding_rules
        )
    else:
    ...
    current_question = (
        initial_plan[0]
        if len(initial_plan) > 0
        else "Let's explore something interesting."
    )
        ...
        exploration_plan_list.append(
            {
                "ref_tables": [
                    {
                        "name": table["name"],
                        "rows": table["rows"][:5] if "rows" in table else [],
                    }
    ...
    while iteration < max_iterations + 1:
        ...
        logger.info(
            f"Iteration {iteration}: Using rec agent for question: {current_question}"
        )

        attempt = 0
        if previous_transformation_dialog:
            if (
                isinstance(previous_transformation_data, dict)
                and "rows" in previous_transformation_data
            ):
            ...
            transformation_results = rec_agent.run(
                input_tables=input_tables, description=current_question
            )
        ...
        yield {
            "iteration": iteration,
            "type": "data_transformation",
            "content": {"question": current_question, "result": transform_result},
            "status": "success",
            "error_message": "",
        }
        ...
        chart_spec = create_chart_spec_from_data(
            transformed_data, chart_type, chart_encodings
        )
        ...
        step_data = {
            ...
            "data": {
                ...
                "name": transformed_data["virtual"]["table_name"]
        ...
        logger.info(
            f"Iteration {iteration}: Using exploration agent to decide next step"
        )
        ...
        if not followup_results or followup_results[0]["status"] != "ok":
            error_msg = (
                followup_results[0]["content"]
                if followup_results
                else "Follow-up planning failed"
            )
            yield {
                "iteration": iteration,
                "type": "planning",
        ...
        exploration_plan_list.append(
            {"ref_tables": last_step_table, "plan": current_plan.copy()}
        )
