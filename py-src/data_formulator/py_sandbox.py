    ...
    conn.send(
        {
            "status": "ok",
            "allowed_objects": {
                key: extended_allowed_objects[key] for key in allowed_objects
            },
        }
    )
    conn.close()
...
def run_in_main_process(code, allowed_objects):
    ...
    for name in [
        ...
        "__import__",
        "Exception",
    ]:  # Note: we need __import__ for importing allowed modules
    ...
    restricted_globals = {"__builtins__": safe_builtins, **allowed_objects}
    ...
    return {
        "status": "ok",
        "allowed_objects": {key: restricted_globals[key] for key in allowed_objects},
    }


def run_transform_in_sandbox2020(code, df_list, exec_python_in_subprocess=False):
    allowed_objects = {"df_list": df_list, "output_df": None}
...
def run_derive_concept(
    code, output_field_name, table_rows, exec_python_in_subprocess=False
):
