...
def table_hash(table):
    ...
    frozen_table = tuple(
        sorted(
            [
                tuple([hash(value_handling_func(r[key])) for key in schema])
                for r in table
            ]
        )
    )
...
def insert_candidates(code, table, dialog, candidate_groups):
    ...
    if t_hash in candidate_groups:
        candidate_groups[t_hash].append(
            {"code": code, "content": table, "dialog": dialog}
        )
...
def dedup_data_transform_candidates(candidates):
    ...
    for candidate in candidates:
        insert_candidates(
            candidate["code"], candidate["data"], candidate["dialog"], candidate_groups
        )
...
def get_field_summary(field_name, df, field_sample_size, max_val_chars=100):
    ...
    try:
        values = sorted(
            [
                make_hashable(x)
                for x in list(set([make_hashable(x) for x in df[field_name].values]))
                if x is not None
            ]
        )
    except:
    ...
    if len(values) <= sample_size:
        ...
        val_sample = (
            values[: int(sample_size / 2)]
            + ["..."]
            + values[-(sample_size - int(sample_size / 2)) :]
        )
...
def generate_data_summary(
    input_tables, include_data_samples=True, field_sample_size=7, max_val_chars=140
):
    ...
    table_summaries = [
        assemble_table_summary(input_table, i)
