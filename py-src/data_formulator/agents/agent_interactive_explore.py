    ...
    def get_data_summary(self, input_tables):
        if self.db_conn:
            ...
            data_summary = generate_data_summary(
                input_tables, include_data_samples=False
            )
        return data_summary
        ...
        if exploration_thread:
            ...
            for i, table in enumerate(exploration_thread, 1):
                ...
                data_summary = self.get_data_summary(
                    [{"name": table_name, "rows": table.get("rows", [])}]
                )
                table_description = table.get("description", "No description available")
                thread_summary += (
                    f"{i}. {table_name}: {table_description} \n\n{data_summary}\n\n"
                )
            context += f"\n\n[EXPLORATION THREAD]\n\n{thread_summary}"
                ...
                messages = [
                    {"role": "system", "content": system_prompt},
                    {
                        ...
                        "content": [
                            ...
                            {
                                "type": "image_url",
                                "image_url": {"url": current_chart, "detail": "high"},
                            },
                        ],
                    },
                ]
