        ...
        with db_manager.connection(session["session_id"]) as db:
            ...
            for table_metadata in table_metadata_list:
                ...
                table_name = (
                    ...
                    else ".".join([database_name, schema_name, table_name])
                )
                ...
                try:
                    ...
                    sample_rows = (
                        db.execute(f"SELECT * FROM {table_name} LIMIT 1000").fetchdf()
                        if row_count > 0
                        else pd.DataFrame()
                    )
                    ...
                    try:
                        ...
                        view_info = db.execute(
                            f"SELECT view_name, sql FROM duckdb_views() WHERE view_name = '{table_name}'"
                        ).fetchone()
                    ...
                    result.append(
                        {
                            ...
                            "sample_rows": json.loads(
                                sample_rows.to_json(orient="records", date_format="iso")
                            ),
                    ...
                    logger.error(
                        f"Error getting table metadata for {table_name}: {str(e)}"
                    )
                    continue
...
def assemble_query(aggregate_fields_and_functions, group_fields, columns, table_name):
    ...
    group_by_clause = (
        ...
        if len(group_fields) > 0 and len(aggregate_fields_and_functions) > 0
        else ""
    )
...
def sample_table():
    ...
    try:
        ...
        aggregate_fields_and_functions = data.get(
            "aggregate_fields_and_functions", []
        )  # each element is a tuple (field, function)
        select_fields = data.get(
            "select_fields", []
        )  # if empty, we want to include all fields
        ...
        with db_manager.connection(session["session_id"]) as db:
            ...
            valid_order_by_fields = [
                field for field in order_by_fields if field in columns
            ]
            valid_aggregate_fields_and_functions = [
                field_and_function
                for field_and_function in aggregate_fields_and_functions
            ...
            query, output_column_names = assemble_query(
                valid_aggregate_fields_and_functions,
                valid_select_fields,
                columns,
                table_id,
            ...
            if method == "random":
                ...
                if valid_order_by_fields:
                    ...
                    order_by_clause = ", ".join(
                        [f'"{field}"' for field in valid_order_by_fields]
                    )
                ...
                if valid_order_by_fields:
                    ...
                    order_by_clause = ", ".join(
                        [f'"{field}" DESC' for field in valid_order_by_fields]
                    )
                    query += f" ORDER BY {order_by_clause} LIMIT {sample_size}"
                else:
...
def get_table_data():
    ...
    try:
        with db_manager.connection(session["session_id"]) as db:
            ...
            if not table_name:
                return jsonify(
                    {"status": "error", "message": "Table name is required"}
                ), 400
            ...
            # Get column names
            columns = [
                col[0] for col in db.execute(f"DESCRIBE {table_name}").fetchall()
            ]
...
def create_table():
    ...
    try:
        if "file" not in request.files and "raw_data" not in request.form:
            return jsonify(
                {"status": "error", "message": "No file or raw data provided"}
            ), 400

        table_name = request.form.get("table_name")
        if not table_name:
            return jsonify(
                {"status": "error", "message": "No table name provided"}
            ), 400
        ...
        with db_manager.connection(session["session_id"]) as db:
            ...
            db.execute(
                "DROP VIEW df_temp"
            )  # Drop the temporary view after creating the table
            ...
            return jsonify(
                {
                    ...
                    "is_renamed": base_name
                    != sanitized_table_name,  # Flag indicating if name was changed
                }
            )
...
def drop_table():
    ...
    try:
        ...
        with db_manager.connection(session["session_id"]) as db:
            ...
            view_exists = (
                db.execute(
                    f"SELECT view_name FROM duckdb_views() WHERE view_name = '{table_name}'"
                ).fetchone()
                is not None
            )
            if view_exists:
            ...
            return jsonify(
                {"status": "success", "message": f"Table/view {table_name} dropped"}
            )
    ...
    try:
        ...
        if not file.filename.endswith(".db"):
            return jsonify(
                {
                    "status": "error",
                    "message": "Invalid file format. Only .db files are supported",
                }
            ), 400
        ...
        try:
            ...
            return jsonify(
                {"status": "error", "message": f"Invalid DuckDB database file."}
            ), 400
...
def download_db_file():
    ...
    try:
        ...
        if not os.path.exists(db_file_path):
            return jsonify(
                {"status": "error", "message": "Database file not found"}
            ), 404
...
def reset_db_file():
    ...
    try:
        ...
        return jsonify(
            {"status": "success", "message": "Database file reset successfully"}
        )
        ...
        with db_manager.connection(session["session_id"]) as db:
            ...
            for col in columns:
                ...
                if col_type in ["INTEGER", "DOUBLE", "DECIMAL"]:
                    ...
                    stats_dict = dict(
                        zip(["count", "unique_count", "null_count"], col_stats)
                    )

                stats.append(
                    {"column": col_name, "type": col_type, "statistics": stats_dict}
                )

        return jsonify(
            {"status": "success", "table_name": table_name, "statistics": stats}
        )
...
def sanitize_table_name(table_name: str) -> str:
    ...
    sanitized_table_name = "".join(
        c for c in sanitized_table_name if c.isalnum() or c == "_"
    )
    ...
    try:
        ...
        if data_loader_type not in DATA_LOADERS:
            return jsonify(
                {
                    "status": "error",
                    "message": f"Invalid data loader type. Must be one of: {', '.join(DATA_LOADERS.keys())}",
                }
            ), 400

        with db_manager.connection(session["session_id"]) as duck_db_conn:
            data_loader = DATA_LOADERS[data_loader_type](
                data_loader_params, duck_db_conn
            )

            # Pass table_filter to list_tables if the data loader supports it
            if (
                hasattr(data_loader, "list_tables")
                and "table_filter" in data_loader.list_tables.__code__.co_varnames
            ):
...
def data_loader_view_query_sample():
    ...
    try:
        ...
        safe_msg, status_code = sanitize_db_error_message(e)
        return jsonify(
            {"status": "error", "sample": [], "message": safe_msg}
        ), status_code
